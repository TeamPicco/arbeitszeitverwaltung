"""
Import-Skript für Inventur-Artikel
Importiert Kategorien und Artikel aus der Inventurliste Piccolo
Einmalig nach Deployment ausführen!
"""
...
def import_inventur_daten(betrieb_id: str):
    """Importiert Kategorien und Artikel für einen Betrieb"""
    
    supabase = get_supabase_client()
    
    print(f"Importiere Inventur-Daten für Betrieb {betrieb_id}...")
    
    # Importiere Kategorien
    kategorie_ids = {}
    
    for kat_data in KATEGORIEN:
        print(f"  Erstelle Kategorie: {kat_data['name']}")
    ...
    for kategorie_name, artikel_liste in ARTIKEL.items():
        ...
        print(f"  Importiere {len(artikel_liste)} Artikel für {kategorie_name}...")
    ...
    print(f"✅ {artikel_count} Artikel erstellt")
    print(f"✅ Import abgeschlossen!")


if __name__ == "__main__":
    # Betrieb-ID von Piccolo (muss angepasst werden!)
    # TODO: Betrieb-ID aus Datenbank holen oder als Argument übergeben
    
    print("=" * 60)
    print("INVENTUR-IMPORT FÜR STEAKHOUSE PICCOLO")
    print("=" * 60)
    print()
    ...
    if not betrieb.data or len(betrieb.data) == 0:
        print("❌ Betrieb Piccolo nicht gefunden!")
        print("Bitte zuerst Betrieb anlegen oder Betriebsnummer prüfen.")
        sys.exit(1)
    
    betrieb_id = betrieb.data[0]['id']
    print(f"Betrieb gefunden: {betrieb_id}")
    print()
    ...
    if kategorien.data and len(kategorien.data) > 0:
        print(f"⚠️  WARNUNG: Es sind bereits {len(kategorien.data)} Kategorien vorhanden!")
    ...
    # Starte Import
    import_inventur_daten(betrieb_id)
