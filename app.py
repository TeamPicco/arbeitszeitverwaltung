...
def _render_login_branding() -> None:
    st.markdown(
        """
        <style>
        .coreo-login-title {text-align:center; font-size:2.1rem; font-weight:700; margin:0.25rem 0 0.1rem 0; color:#ffffff;}
        .coreo-login-tagline {text-align:center; color:#cbd5e1; margin:0 0 1.2rem 0;}
    ...
    with center:
        if BRAND_LOGO_IMAGE:
            st.image(BRAND_LOGO_IMAGE, width=320)
        st.markdown(f"<div class='coreo-login-title'>{BRAND_APP_NAME}</div>", unsafe_allow_html=True)
        if BRAND_TAGLINE:
            st.markdown(f"<div class='coreo-login-tagline'>{BRAND_TAGLINE}</div>", unsafe_allow_html=True)
...
if not st.session_state.get('logged_in'):
    ...
    _, login_center, _ = st.columns([1, 1.35, 1])
    with login_center:
